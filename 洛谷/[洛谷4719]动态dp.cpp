...
struct Matrix
{
    int b[3][3];
    Matrix () {}
    inline Matrix (int _v1,int _v2,int _v3,int _v4)
    {
        b[1][1]=_v1;b[1][2]=_v2;
        b[2][1]=_v3;b[2][2]=_v4;
    }
    inline void clear() {for (register int i=1;i<=2;i++) for (register int j=1;j<=2;j++) b[i][j]=-INF;}
        ...
        for (register int i=1;i<=2;i++)
            for (register int j=1;j<=2;j++)
                for (register int k=1;k<=2;k++)
                    check_max(res.b[i][j],b[i][k]+other.b[k][j]);
        return res;
    }
};
...
int depth[MAXN+48],fa[MAXN+48],son[MAXN+48],sz[MAXN+48];
int top[MAXN+48],pos[MAXN+48],ed[MAXN+48],totree[MAXN+48],ind;
...
inline void dfs(int cur,int father)
{
    ...
    for (auto y : v[cur])
        if (y!=father)
        {
            f[cur][1]+=f[y][0];f[cur][0]+=max(f[y][0],f[y][1]);
            if (y!=son[cur]) g[cur][1]+=f[y][0],g[cur][0]+=max(f[y][0],f[y][1]);
        }
...
namespace SegmentTree
{
    ...
    {
        info[cur]=info[cur<<1|1]*info[cur<<1];
    }
    inline void build(int cur,int l,int r)
    {
        ...
        {
            int pt=totree[l];
            info[cur]=Matrix(g[pt][0],g[pt][1],g[pt][0],-INF);
            return;
        }
        int mid=(l+r)>>1;
...
inline void update(int u,int nv)
{
    g[u][1]+=nv-val[u];val[u]=nv;Matrix od,nw;
    while (u)
    {
        od=query(top[u]);
        ...
        nw=query(top[u]);
        u=fa[top[u]];
        g[u][0]+=max(nw.b[1][1],nw.b[1][2])-max(od.b[1][1],od.b[1][2]);
        g[u][1]+=nw.b[1][1]-od.b[1][1];
    }
}
...
int main ()
{
    ...
    dfs(1,-1);SegmentTree::build(1,1,n);
    while (m--)
    {
        scanf("%d%d",&x,&y);update(x,y);
        Matrix tmp=query(1);
        printf("%d\n",max(tmp.b[1][1],tmp.b[1][2]));
    }
    io.flush();
#ifdef LOCAL    
