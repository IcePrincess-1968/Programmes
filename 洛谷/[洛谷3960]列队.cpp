...
namespace Treap
{
	...
	{
		if (!root) return k;
		int leftsz=tree[root].val-tree[tree[root].left].sz-1;
		if (leftsz>=k) return find(tree[root].left,k); else return tree[root].val+find(tree[root].right,k-leftsz);
	}
}
...
int main ()
{
	...
	while (q--)
	{
		...
		if (y<=m-1-cnt[x])
		{
			/*LL res=1ll*(x-1)*m+y+Treap::Count(root1[x],y);
			printf("%lld\n",res);
			Treap::treap_insert(root1[x],y);
			LL tmp=Treap::query(Root,x);Treap::treap_delete(Root,x);
			Treap::treap_insert(root2[x],++cnt[x],tmp);
			Treap::treap_insert(Root,n,res);*/
			LL res=Treap::find(root1[x],y);
			printf("%lld\n",1ll*(x-1)*m+res);
