...
namespace splay
{
	...
	inline void update(int cur) {tree[cur].sz=tree[cur].cnt+tree[tree[cur].ch[0]].sz+tree[tree[cur].ch[1]].sz;}
	...
	inline int find(int val)
	{
		...
		for (;;)
		{
			if (tree[cur].val==val) {splay(cur,0);break;}
			if (val<tree[cur].val)
			{
	...
	inline void del(int val)
	{
		...
		tree[root].ch[1]=tree[oldroot].ch[1];tree[root].father=0;
